...
//Declare variables
int transmissinrate=960,filesize;
...
time_in_seconds=static_cast<double>(filesize)/transmissinrate;

//converting the time in seconds in to hours and minutes
time_in_minutes=static_cast<int>(ceil(time_in_seconds/60));
time_in_hours=static_cast<int>(ceil(time_in_seconds/60));

cout<<"The time takes to send the file size of " <<filesize<< " bytes with transmissin rate of "<<transmissinrate<<" character/second is "<<endl;
